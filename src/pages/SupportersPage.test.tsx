...
function state(overrides = {}) {
  vi.mocked(useSupporter).mockReturnValue({ isActive: false, isSignedIn: true, canCheck: true, isFetching: false, isError: false, data: undefined, refetch: vi.fn(), ...overrides } as unknown as ReturnType<typeof useSupporter>);
}
beforeEach(async () => { await initializeI18n({ force: true, languages: ['en-US'] }); state(); });
describe('SupportersPage', () => {
  ...
  it('promotes native purchase and explains the verification prerequisite for inactive accounts', () => {
    ...
    expect(screen.getByRole('link', { name: 'Get the app to become a supporter' })).toHaveAttribute('href', '/download');
  });
