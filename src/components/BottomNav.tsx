import { House as Home, Compass, MagnifyingGlass as Search, Bell, User, TrendUp, Plus } from '@phosphor-icons/react';
      ...
      <div className="flex items-center justify-around h-16 px-2">
        ...
        {user && (
          <Button
            ...
            className={cn(
              "flex flex-col items-center justify-center gap-1 h-full flex-1 rounded-none hover:bg-transparent",
              isActive('/post') && "text-primary"
            )}
          >
            <Plus className={cn("h-5 w-5", isActive('/post') && "text-primary")} weight={isActive('/post') ? 'fill' : 'bold'} />
            <span className={cn("text-xs", isActive('/post') && "text-primary")}>{t('nav.post', { defaultValue: 'Post' })}</span>
          </Button>
        )}
